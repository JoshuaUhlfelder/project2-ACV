...
"""
The original file did not have a header. You could add one.
Also, all torchvision imports are not used below, so you can delete them
from your import list. Check this for all your code, actually.
"""
...
class MyTrainDataset(torch.utils.data.Dataset):
    ...
    def get_fragment_folders(self, images_dir, index_list):
        ...
        """
        You could integrate the below function into the above by adding
    ...
    def __getitem__(self, idx):  
        ...
        """
        This is good. You moved the mask operations to the top so that if
        ...
        """
        Minor suggestion: you could use torchvision transforms to make this
        flipping process more clear. But This is probably fine. 
        """            
        ...
        #    return None
        """
        COmment above is irrelevant
        """


"""
This class seems to be the same as the one above with the
only different being the resizing.
Try integrating these functiobns together to save sapce.
You could make a new varaible when you instantiate the class
that determines if it is a training or test dataset.
Depending on this variable you can pick indicies and return
the right window size.

This might be slightly more inefficient, but it would
make the code more clear

If not, assume all comments above apply to the below code and
...
class MyValDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):  
        ...
        try:
            ...
            """
            On the previous comment: you are not transforming the validaiton data,
            so that is a pretty big different between the classes. Adds some
            nuance to if you want to integrate these classes
            """
            ...
            return None
        """
        For both classes, it is highly unlikely that you encounter null data
        so you could just get rid of the try statement. 
        Because otherwise you won't return much in your collate function because 
        there are only 3 fragments
        """
...
class MultiDimLinear(torch.nn.Linear):
    ...
    """
    You loose the batch information by reshaping
    to (1,32,32). THis probably prevents you from having
    a batch size above 1. Instead, resize to (1,1,32,32)
    """
    def forward(self, x):
        out = super().forward(x)
...
def score(true_mask, predicted):
    ...
    """
    If you comment out the print statements, you do not need
    to complete the operations above them.
    """
...
"""
Good training loop
One note: if you end up changing the 
model architecture to reshape the output to (1,1,32,32),
make sure you dereference the correct bariable when 
clauclating preds in line 481
"""
